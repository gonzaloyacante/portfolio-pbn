...
export function AboutEditor({ settings }: AboutEditorProps) {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 rounded-lg border bg-white p-6 shadow-sm dark:bg-gray-800">
            {/* Intro Section */}
            <h2 className="text-lg font-semibold">Introducción y Bio</h2>

            <div className="grid gap-6 md:grid-cols-2">
                <div className="space-y-4">
                    <Input label="Título de Presentación" {...register('bioTitle')} error={errors.bioTitle?.message} />
                ...
                <div className="grid gap-6 md:grid-cols-2">
                    ...
                    <div className="space-y-2">
                        ...
                        <textarea
                            ...
                            onChange={(e) => setValue('certifications', e.target.value.split(',').map(s => s.trim()).filter(Boolean))}
                            className="w-full rounded border p-2 text-sm dark:bg-gray-900"
                            rows={4}
                        />
                        <p className="text-xs text-gray-500">Ej: Master en Maquillaje 2023, Curso FX Avanzado</p>
                    </div>
                </div>
            ...
            <div className="border-t pt-6">
                <div className="flex items-center gap-4">
                    ...
                    />
                    <label htmlFor="showTests" className="font-medium">Mostrar Sección de Testimonios</label>
                </div>
