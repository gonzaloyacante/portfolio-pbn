  ...
  return (
    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="bg-card rounded-card overflow-hidden border shadow-sm">
...
export function SkeletonCategoryGrid({ count = 6 }: { count?: number }) {
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="bg-card rounded-card flex items-center gap-4 border p-4 shadow-sm">
...
export function SkeletonTestimonialList({ count = 4 }: { count?: number }) {
  return (
    <div className="space-y-4">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="bg-card rounded-card space-y-3 border p-6 shadow-sm">
...
export function SkeletonContactList({ count = 6 }: { count?: number }) {
  return (
    <div className="bg-card rounded-card overflow-hidden border shadow-sm">
      {Array.from({ length: count }).map((_, i) => (
