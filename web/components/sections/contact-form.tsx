...
// Wrappers para mantener compatibilidad con API antigua
const FormInput = ({ value, onChange, ...props }: any) => (
  <Input 
    {...props} 
    value={value} 
...
const FormTextarea = ({ value, onChange, ...props }: any) => (
  <Textarea 
    {...props} 
    value={value} 
    onChange={(e) => onChange(e.target.value)} 
    ...
    <form onSubmit={handleSubmit} className="space-y-6" aria-label="Formulario de contacto">
      ...
      {submitStatus === "success" && (
        <div 
          role="alert" 
          aria-live="polite" 
          className="p-5 bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-400 rounded-xl text-green-700 animate-scale-in-pop shadow-lg"
        >
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-green-500 rounded-full flex items-center justify-center animate-bounce" aria-hidden="true">
      ...
      {submitStatus === "error" && (
        <div 
          role="alert" 
          aria-live="assertive" 
          className="p-5 bg-gradient-to-r from-red-50 to-rose-50 border-2 border-red-400 rounded-xl text-red-700 animate-shake shadow-lg"
        >
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-red-500 rounded-full flex items-center justify-center" aria-hidden="true">
              <svg className="w-6 h-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
