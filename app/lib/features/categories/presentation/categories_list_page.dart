...
class _CategoriesListPageState extends ConsumerState<CategoriesListPage> {
  ...
  Future<void> _delete(BuildContext ctx, CategoryItem item) async {
    ...
    try {
      ...
      if (ctx.mounted) {
        ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text('Categoría eliminada')));
      }
    } catch (e, st) {
  ...
  Widget build(BuildContext context) {
    final async = ref.watch(categoriesListProvider(search: _search.isEmpty ? null : _search));
      ...
      body: Column(
        children: [
          Padding(
            ...
            child: AppSearchBar(hint: 'Buscar categorías…', controller: _searchController, onChanged: _onSearch),
          ),
          Expanded(
            child: async.when(
              loading: () => const _CategoriesSkeleton(),
              error: (e, _) => ErrorState(message: e.toString(), onRetry: () => ref.invalidate(categoriesListProvider)),
                  ...
                  : RefreshIndicator(
                      ...
                      child: ListView.separated(
                        ...
                        itemBuilder: (ctx, i) => FadeSlideIn(
                          ...
                          child: _CategoryTile(item: paginated.data[i], onDelete: _delete),
                        ),
                      ),
                    ),
            ),
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: InkWell(
        ...
        onTap: () => context.pushNamed(RouteNames.categoryEdit, pathParameters: {'id': item.id}),
        child: Padding(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Icon(Icons.link_rounded, size: 13, color: scheme.outline),
                        const SizedBox(width: 4),
                        Expanded(
                          child: Text(
                            '/${item.slug}',
                        ...
                        Icon(Icons.photo_library_outlined, size: 13, color: scheme.outline),
              ...
              PopupMenuButton<String>(
                ...
                onSelected: (action) {
                  if (action == 'edit') {
                    context.pushNamed(RouteNames.categoryEdit, pathParameters: {'id': item.id});
