...
extension _CategoryFormPageBuilders on _CategoryFormPageState {
          ...
          child: Form(
            ...
            child: _isEdit
                ? RefreshIndicator(
                    ...
                    child: _buildFormListView(context, imageHeight),
                  )
                : _buildFormListView(context, imageHeight),
          ),
        ),
      ),
    );
  ...
  Widget _buildFormListView(BuildContext context, double imageHeight) {
    final mediaSize = MediaQuery.sizeOf(context);
    return ListView(
  ...
  Future<void> _refreshFromServer() async {
    if (!_isDirty) setState(() => _populatedFor = null);
    ref.invalidate(categoryDetailProvider(widget.categoryId!));
    await ref.read(categoryDetailProvider(widget.categoryId!).future);
  }
