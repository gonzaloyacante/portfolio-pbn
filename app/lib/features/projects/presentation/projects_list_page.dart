...
class _ProjectsList extends ConsumerStatefulWidget {
  const _ProjectsList({
  ...
  @override
  ConsumerState<_ProjectsList> createState() => _ProjectsListState();
}

class _ProjectsListState extends ConsumerState<_ProjectsList> {
  ...
  Future<void> _loadNextPage() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Error cargando más proyectos')));
      }
    }
  }
  ...
  Widget build(BuildContext context) {
      ...
      return GridView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return FadeSlideIn(
            ...
            child: _ProjectGridCard(item: item, onDelete: widget.onDelete),
          );
        },
      );
    }
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        return FadeSlideIn(
          delay: Duration(milliseconds: (index * 40).clamp(0, 300)),
          child: _ProjectTile(item: item, onDelete: widget.onDelete),
        );
      },
    );
  }
