...
void main() {
  ...
  group('PaginationMeta.fromJson — required fields', () {
    ...
    test('parses page', () => expect(PaginationMeta.fromJson(meta()).page, 1));
    test(
      'parses limit',
      () => expect(PaginationMeta.fromJson(meta()).limit, 10),
    );
    test(
      'parses total',
      () => expect(PaginationMeta.fromJson(meta()).total, 50),
    );
    test(
      'parses totalPages',
      () => expect(PaginationMeta.fromJson(meta()).totalPages, 5),
    );
    test(
      'hasNext defaults false',
      () => expect(PaginationMeta.fromJson(meta()).hasNext, isFalse),
    );
    test(
      'hasPrev defaults false',
      () => expect(PaginationMeta.fromJson(meta()).hasPrev, isFalse),
    );
  });
  ...
  group('PaginationMeta — equality', () {
    test('same values are equal', () {
      final a = PaginationMeta.fromJson({
        'page': 1,
        'limit': 10,
        'total': 100,
        'totalPages': 10,
      });
      final b = PaginationMeta.fromJson({
        'page': 1,
        'limit': 10,
        'total': 100,
        'totalPages': 10,
  ...
  group('PaginationMeta — copyWith', () {
    final base = PaginationMeta.fromJson({
      'page': 1,
      'limit': 10,
      'total': 100,
      'totalPages': 10,
    ...
    test(
      'copyWith hasNext',
      () => expect(base.copyWith(hasNext: true).hasNext, isTrue),
    );
    test(
      'copyWith hasPrev',
      () => expect(base.copyWith(hasPrev: true).hasPrev, isTrue),
    );
    test('original unchanged after copyWith', () {
      final _ = base.copyWith(page: 99);
      expect(base.page, 1);
    });
  });
  ...
  group('PaginatedResponse<String>.fromJson', () {
    Map<String, dynamic> resp(List<String> items) => {
      'data': items,
      'pagination': {
        'page': 1,
        'limit': 10,
    ...
    test('parses data list', () {
      final r = PaginatedResponse<String>.fromJson(
        resp(['a', 'b', 'c']),
        (e) => e as String,
      );
      expect(r.data, ['a', 'b', 'c']);
  ...
  group('PaginatedResponse<Map<String, dynamic>>.fromJson', () {
